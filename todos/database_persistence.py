...
class DatabasePersistence:
    ...
    def _database_connect(self):
        if os.environ.get('FLASK_ENV') == 'production':
            ...
            login = f"dbname=todos user=postgres password={secrets['db_password']}"
    ...
    def update_list_by_id(self, list_id, title):
       ...
       logger.info("Executing query: %s, with list_id: %s, with title: %s", query, list_id, title)
       with self._database_connect() as conn:
           with conn.cursor(cursor_factory=DictCursor) as cursor:
    ...
    def create_new_todo(self, list_id, todo_title):
        ...
        logger.info("Executing query: %s with list_id: %s with todo_title: %s", query, list_id, todo_title)
        with self._database_connect() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cursor:
    ...
    def delete_todo_by_id(self, todo_id, list_id):
        ...
        logger.info("Executing query: %s with todo_id: %s and list_id: %s", query, todo_id, list_id)
        with self._database_connect() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cursor:
                cursor.execute(query, (todo_id, list_id))

    def update_todo_status(self, list_id, todo_id, is_completed):
        query = "UPDATE todos SET completed = %s WHERE id = %s AND list_id = %s"    
        logger.info("Executing query: %s with list_id: %s, todo_id: %s, and completed: %s",
                query, list_id, todo_id, is_completed)
        with self._database_connect() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cursor:
